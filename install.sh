...
EOF
        ...
        sed -i -E 's|^analyticsApiUrl\s*=\s*"http://127\.0\.0\.1:[0-9]+";|analyticsApiUrl = "";|' "$TMP/config.js" 2>/dev/null || true
        sed -i -E 's|^analyticsApiUrl\s*=\s*"http://localhost:[0-9]+";|analyticsApiUrl = "";|' "$TMP/config.js" 2>/dev/null || true
    fi
